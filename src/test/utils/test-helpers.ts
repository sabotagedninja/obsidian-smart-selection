...
export function findCaretIndexes(str: string): CaretIndexes {
    const containsOnlyOnePipeCaret = str.indexOf('|') === str.lastIndexOf('|') && !str.contains('^');
    const containsOnlyTwoPipeCarets = str.indexOf('|') < str.lastIndexOf('|') && !str.contains('^');
...
export function expand(plugin: SelectionExpanderPluginImpl, editor: Editor, textWithCarets: string) {
